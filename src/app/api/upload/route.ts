...
async function getNextSequentialNumber(baseName: string, extension: string): Promise<number> {
  ...
  existingProposals.forEach(proposal => {
    ...
    // Check for numbered versions: basename_1.tsx, basename_2.tsx, etc.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.name.endsWith('.tsx')) {
      return NextResponse.json(
        { message: 'Only .tsx files are allowed' },
        { status: 400 }
      );
    }
    ...
    if (!content.includes('export default') && !content.includes('export {')) {
      return NextResponse.json(
        { message: 'File must contain a default export (React component)' },
        { status: 400 }
      );
    }

    // Generate unique filename with sequential numbering
    const originalName = file.name.replace(/\.tsx$/, '');
    const extension = '.tsx';
